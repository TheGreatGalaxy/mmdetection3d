...
class LoadPointsFromMultiSweeps(object):
    ...
    def __call__(self, results):
        ...
        # print("load points from multisweeps: ", self.__repr__())
        ...
        # Nuscenes data dim: x, y, z, i, delta_time_s.
        ...
        if self.normalize_intensity is not None:
            if isinstance(self.normalize_intensity, int) and self.normalize_intensity in self.use_dim:
                points.tensor[:, self.normalize_intensity] /= 255.0
            else:
                print(
                    "normalize_intensity must be a int, and must in use_dim or range(use_dim)")
                raise
...
class LoadPointsFromFileCustom(object):
    ...
    def __call__(self, results):
        ...
        if self.normalize_intensity is not None:
            if isinstance(self.normalize_intensity, int) and self.normalize_intensity in self.use_dim:
                points[:, self.normalize_intensity] /= 255.0
            else:
                print(
                    "normalize_intensity must be a int, and must in use_dim or range(use_dim)")
                raise
