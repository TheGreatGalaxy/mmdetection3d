...
def _draw_points_o3dvisualizer(points,
                               vis,
                               points_size=2,
                               point_color=(0.5, 0.5, 0.5),
                               mode='xyz'):
    ...
    Args:
        points (numpy.array | torch.tensor, shape=[N, 3+C]):
            points to visualize.
        vis (:obj:`open3d.visualization.O3DVisualizer`): open3d O3DVisualizer.
        points_size (int): the size of points to show on O3DVisualizer.
            Default: 2.
        point_color (tuple[float]): the color of points.
            Default: (0.5, 0.5, 0.5).
        mode (str):  indicate type of the input points, avaliable mode
    ...
    Returns:
        tuple: points, color of each point.
    """
    vis.point_size = points_size  # set points size
    if isinstance(points, torch.Tensor):
        points = points.cpu().numpy()
...
def _draw_bboxes_with_descriptions(bbox3d,
                                   descriptions,
                                   vis,
                                   points_colors,
                                   pcd=None,
                                   ...
                                   center_mode='lidar_bottom',
                                   mode='xyz'):
    """Draw bbox with descriptions on visualizer and change the color of points inside bbox3d.

    Args:
        bbox3d (numpy.array | torch.tensor, shape=[M, 7]):
            3d bbox (x, y, z, dx, dy, dz, yaw) to visualize.
        descriptions: list[str], len should be consisted with bbox3d num.
        vis (:obj:`open3d.visualization.O3DVisualizer`): open3d O3DVisualizer.
        points_colors (numpy.array): color of each points.
        pcd (:obj:`open3d.geometry.PointCloud`): point cloud. Default: None.
        bbox_color (tuple[float]): the color of bbox. Default: (0, 1, 0).
    ...
    in_box_color = np.array(points_in_box_color)
    assert(len(bbox3d) == len(descriptions))
    for i in range(len(bbox3d)):
        center = bbox3d[i, 0:3]
        dim = bbox3d[i, 3:6]
        yaw = np.zeros(3)
        ...
        line_set.paint_uniform_color(bbox_color)
        # draw bboxes on visualizer
        name = "bbox" + str(center[0]) + str(center[1])
        vis.add_geometry(name, line_set)
        vis.add_3d_label(
            np.array([center[0], center[1], center[2]]), descriptions[i])
...
class O3DVisualizer(object):
    ...
    def __init__(self,
        ...
        self.o3d_visualizer = o3d.visualization.O3DVisualizer(
            width=1920, height=1080)
        self.o3d_visualizer.setup_camera(180.0, np.array([0.0, 0.0, 0.0]),
                                         np.array([0.0, 0.0, 40.0]), np.array([0.0, 0.0, 1.0]))
        self.o3d_visualizer.show_skybox(False)
        self.o3d_visualizer.ground_plane = rendering.Scene.GroundPlane.XY
        self.o3d_visualizer.show_axes = True
        # if open3d version >= 0.15.2
        self.o3d_visualizer.enable_raw_mode(True)
        self.app.add_window(self.o3d_visualizer)
       ...
       # draw points
        if points is not None:
            self.pcd, self.points_colors = _draw_points_o3dvisualizer(
                points, self.o3d_visualizer, points_size, point_color, mode)

        # draw boxes
        if bbox3d is not None and descriptions is not None:
            assert(len(bbox3d) == len(descriptions))
            _draw_bboxes_with_descriptions(bbox3d, descriptions, self.o3d_visualizer, self.points_colors,
                                           self.pcd, bbox_color, points_in_box_color, rot_axis,
                                           center_mode, mode)

    def add_bboxes(self, bbox3d, descriptions, bbox_color=None, points_in_box_color=None):
        """Add bounding box to visualizer.
        ...
        if points_in_box_color is None:
            points_in_box_color = self.points_in_box_color
        _draw_bboxes_with_descriptions(bbox3d, descriptions, self.o3d_visualizer, self.points_colors,
                                       self.pcd, bbox_color, points_in_box_color, self.rot_axis,
                                       self.center_mode, self.mode)
